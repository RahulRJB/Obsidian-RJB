...
- #### Solution:
	...
	- The chat history was to be saved for a given session for a later use.
		- A session could also have multiple topics selected at the same time or at different points of time, in which case independent retrieval was done across the diff indices and then the retrieved documents from the different indices were reranked using [[Notes/RAG#|RAG]].
- #### xxxxxxxxxxxx:
